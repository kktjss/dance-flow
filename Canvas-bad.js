...
const Canvas = ({
    ...
    const forceUpdateObjectPositions = useCallback(() => {
        ...
        fabricCanvas.getObjects().forEach(obj => {
            if (obj.data && obj.data.element) {
                ...
                obj.selectable = true; // Always allow selection
                obj.evented = true; // Always allow events
                obj.hasControls = !readOnly; // Only show controls in edit mode
                obj.hasBorders = !readOnly; // Only show borders in edit mode
                obj.lockMovementX = readOnly; // Lock movement in readOnly mode
                obj.lockMovementY = readOnly; // Lock movement in readOnly mode
            }
        });
    ...
    useEffect(() => {
        ...
        canvasContainer.style.pointerEvents = 'auto'; // Always allow pointer events
        ...
        setTimeout(() => {
            ...
            if (readOnly) {
                // Allow selection in readOnly mode but disable editing
    ...
    const setupEventHandlers = useCallback((fabricCanvas) => {
        ...
        if (readOnly) {
            // Only set up selection events in readOnly mode
            fabricCanvas.on('mouse:down', () => {
                fabricCanvas.__isMousedown = true;
            });

            fabricCanvas.on('mouse:up', () => {
                fabricCanvas.__isMousedown = false;
            ...
            // Selection handlers for readOnly mode
        ...
        // Добавляем обработчики для отслеживания состояния мыши
        fabricCanvas.on('mouse:down', () => {
            fabricCanvas.__isMousedown = true;
        });
        ...
        fabricCanvas.on('object:modified', (e) => {
            ...
            // Отмечаем время модификации
            modifiedObject.lastManualMoveTime = Date.now();
            ...
            // Обновляем значения в базе данных без лишних console.log
            if (onElementsChange) {
                onElementsChange(updatedElements);
            }
        });

        // Добавляем обработчик для обновления позиции во время перемещения
        fabricCanvas.on('object:moving', (e) => {
            const movingObject = e.target;

            if (!movingObject || !movingObject.data || !movingObject.data.elementId) {
                return;
            }

            // Устанавливаем время последнего перемещения
            movingObject.lastManualMoveTime = Date.now();
            ...
            // Обновляем элемент с текущими координатами объекта
            const updatedElements = effectiveElements.map(elem => {
                if (elem.id === elementId) {
                    return {
                        ...elem,
                        position: {
            ...
            // Если это выбранный элемент, обновляем его данные для панели свойств
            if (selectedElement && selectedElement.id === elementId && onElementSelect) {
        ...
        // Обработчик выбора элемента - обновляем данные для панели свойств
        fabricCanvas.on('selection:created', (e) => {
            if (e.selected && e.selected.length > 0) {
                const selectedObject = e.selected[0];
                ...
                if (selectedObject.data && selectedObject.data.elementId) {
                    let element = effectiveElements.find(el => el.id === selectedObject.data.elementId);
                    if (element) {
                        // Обновляем с актуальной позицией объекта
                        element = {
                            ...element,
                            position: {
        ...
        fabricCanvas.on('mouse:down', (e) => {
            if (e.target) {
                // Устанавливаем Z-индекс выбранного объекта выше остальных
    ...
    const applyAnimationEffects = useCallback((fabricObject, element) => {
        ...
        // Никогда не пытаемся анимировать объект, который в данный момент активен (выбран)
        if (fabricObject.canvas && fabricObject === fabricObject.canvas.getActiveObject()) {
            return;
        }

        // Если объект недавно перемещался вручную, не трогаем его
        const now = Date.now();
        const recentMoveTime = fabricObject.lastManualMoveTime || 0;
        const moveCooldown = 1000; // Увеличиваем до 1 секунды для надежности

        if (now - recentMoveTime < moveCooldown) {
            return;
        }
        ...
        // Устанавливаем новые свойства объекта
        fabricObject.set({
            left: currentX,
            top: currentY,
            opacity: currentOpacity,
            ...
            scaleY: currentScale,
            visible: true,
            lockMovementX: readOnly, // Lock movement in readOnly mode
            lockMovementY: readOnly  // Lock movement in readOnly mode
        });
        fabricObject.setCoords();

        // Ensure the object is clickable
        fabricObject.selectable = true; // Always allow selection
        fabricObject.evented = true; // Always allow events
        fabricObject.hasControls = !readOnly; // Only show controls in edit mode
        fabricObject.hasBorders = !readOnly; // Only show borders in edit mode
        fabricObject.lockMovementX = readOnly; // Lock movement in readOnly mode
        fabricObject.lockMovementY = readOnly; // Lock movement in readOnly mode
    ...
    useEffect(() => {
        ...
        const updateCanvas = () => {
            try {
                        ...
                        if (fabricObject) {
                            ...
                            switch (element.type) {
                                case 'rectangle':
                                    ...
                                    // Установим z-индекс объекта в соответствии с его порядком в массиве
                                    fabricObject.zIndex = index;
                ...
                // Add new objects to canvas (сортируем по z-индексу для правильного порядка)
                objectsToAdd.sort((a, b) => a.zIndex - b.zIndex).forEach(obj => {
        ...
        const animateCanvas = (timestamp) => {
            ...
            // Ограничиваем частоту обновлений для повышения производительности
            const timeSinceLastUpdate = timestamp - lastAnimationTime;
            const updateIntervalMs = 33; // ~30fps для более плавной работы

            // Проверяем, есть ли активное перетаскивание
            const fabricCanvas = fabricInstances.get(canvasId.current);
            const isMouseDown = fabricCanvas.__isMousedown;

            // Если мышь нажата, не обновляем анимацию, чтобы не мешать пользователю
            if (isMouseDown) {
                // Просто продолжаем цикл анимации без обновлений
                animationFrame = requestAnimationFrame(animateCanvas);
                return;
            }

            // Обновляем анимацию при изменении времени или по таймеру
            if (timeChanged || timeSinceLastUpdate > updateIntervalMs) {
                lastAnimationTime = timestamp;

                // Получаем активный объект
                const activeObject = fabricCanvas.getActiveObject();

                // Обновляем только объекты с ключевыми кадрами, 
                // и только если они не выбраны в данный момент
                let animationApplied = false;
                fabricCanvas.getObjects().forEach(obj => {
                    if (obj.data && obj.data.element) {
                        const element = obj.data.element;

                        // Пропускаем выбранный объект
                        if (obj === activeObject) {
                            return;
                        }

                        // Применяем анимацию только к элементам с ключевыми кадрами
                        if (element.keyframes && element.keyframes.length > 0) {
                            const result = applyAnimationEffects(obj, element);
                            if (result) animationApplied = true;
                        }
                    }
                });

                // Обновляем canvas только если была применена анимация
                if (animationApplied || timeChanged) {
                    fabricCanvas.renderAll();
                }
            }

            // Всегда продолжаем цикл анимации
            animationFrame = requestAnimationFrame(animateCanvas);
        };
    ...
    }, [initialized, effectiveElements, currentTime, selectedElement, isPlaying, setupEventHandlers, applyAnimationEffects, readOnly, forceUpdateObjectPositions, isRecordingKeyframes]);
    ...
    // ВРЕМЕННО: Рендер кнопки просмотра 3D - будет удалено позже (было: "Рендер кнопки просмотра 3D/видео")
    ...
    // Добавляем эффект для обновления анимации при изменении времени
    useEffect(() => {
        // Принудительно обновляем все объекты с ключевыми кадрами при изменении времени
        if (!initialized || !fabricInstances.has(canvasId.current)) return;

        console.log(`Time changed to ${currentTime}, updating keyframe animations`);

        const fabricCanvas = fabricInstances.get(canvasId.current);
        const activeObject = fabricCanvas.getActiveObject();

        // Не обновляем объекты, если идет активное перетаскивание
        if (fabricCanvas.__isMousedown && activeObject) {
            console.log('Skipping updates during active dragging');
            return;
        }

        // Обновляем все объекты с ключевыми кадрами
        fabricCanvas.getObjects().forEach(obj => {
            if (obj.data && obj.data.element) {
                const element = obj.data.element;

                // Пропускаем выбранный объект в режиме записи ключевых кадров
                if (isRecordingKeyframes && obj === activeObject) {
                    console.log(`Skipping animation for selected object ${element.id} in recording mode`);
                    return;
                }
        ...
        // Рендерим canvas
        fabricCanvas.renderAll();
    }, [currentTime, initialized, canvasId, applyAnimationEffects, isRecordingKeyframes]);

    // Экспортируем функцию для обновления позиции из панели свойств
    useEffect(() => {
        if (typeof window !== 'undefined') {
            // Создаем глобальную функцию для обновления позиции элемента из панели свойств
            window.updateElementPosition = (elementId, x, y) => {
                console.log(`Canvas: Got request to update element ${elementId} position to (${x},${y})`);

                if (!fabricInstances.has(canvasId.current)) {
                    console.warn('Canvas instance not found');
                    return false;
                }

                const fabricCanvas = fabricInstances.get(canvasId.current);

                // Находим объект на canvas
                const obj = fabricCanvas.getObjects().find(o =>
                    o.data && o.data.elementId === elementId
                );

                if (!obj) {
                    console.warn(`Object with ID ${elementId} not found on canvas`);
                    return false;
                }

                // Сохраняем время последнего ручного изменения
                obj.lastManualMoveTime = Date.now();
                ...
                const updatedElements = effectiveElements.map(elem => {
                    if (elem.id === elementId) {
                        // Обновляем базовую позицию элемента
                        const updatedElement = {
                            ...elem,
                            position: { x, y }
                        };

                        // Если у элемента есть ключевые кадры и мы в режиме записи, 
                        // создаем новый ключевой кадр
                        if (isRecordingKeyframes &&
                            updatedElement.keyframes &&
                            updatedElement.keyframes.length > 0) {

                            console.log(`Creating keyframe at time ${currentTime} from property panel`);
                ...
                // Обновляем кэш анимированных позиций
                if (!lastAnimatedPositionsRef.current[elementId]) {
                    lastAnimatedPositionsRef.current[elementId] = {};
                }
                ...
                // Если это выбранный элемент, обновляем также selectedElement
                ...
                console.log(`Updated element ${elementId} position to (${x},${y})`);
                return true;
            };
        }
        ...
        return () => {
            if (typeof window !== 'undefined') {
                delete window.updateElementPosition;
            }
        };
    }, [canvasId, effectiveElements, selectedElement, onElementsChange, onElementSelect, currentTime, lastAnimatedPositionsRef, isRecordingKeyframes, addOrUpdateKeyframe]);
    ...
    return (
        ...
        >
            ...
            {/* ВРЕМЕННО: Рендерим кнопку просмотра 3D - будет удалено позже (было: "Рендерим кнопку просмотра 3D/видео") */}
            {renderViewerButton()}

            {/* ВРЕМЕННО: Модальное окно выбора хореографии - будет удалено позже */}
            {showChoreoModal && renderChoreoModal()}

            {/* ВРЕМЕННО: Просмотрщик 3D моделей - будет удален позже (было: "Комбинированный просмотрщик 3D и видео") */}
