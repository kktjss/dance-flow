...
describe('ProtectedRoute Component', () => {
    ...
    // Тест: пользователь с истекшим токеном должен быть перенаправлен
    it('должен перенаправлять, если токен есть, но пользователь не аутентифицирован', () => {
        // Создаем мок-хранилище с ситуацией "токен есть, но пользователь не аутентифицирован"
        const store = mockStore({
            auth: {
                isAuthenticated: false, // Не аутентифицирован
                token: 'expired-token', // Но токен есть (истекший)
                user: null
            }
        });
